...
func newTestConfig(t *testing.T, db *sql.DB, syntax PlaceholderSyntax) *Config {
	...
	return NewConfig(
		NewDatabase(clock, logger, NewRepository(db, logger, syntax), DefaultStatements{}),
		newTestMigrations(t),
	).
		WithClock(clock).
		WithLogger(logger)
...
func TestNewConfig(t *testing.T) {
	logger := newTestLogger(t)
	db := NewDatabase(newTestClock(), logger, NewRepository(nil, logger, PlaceholderIndexed), DefaultStatements{})
...
func TestConfig_WithClock(t *testing.T) {
	...
	cfg := NewConfig(
		NewDatabase(newTestClock(), logger, NewRepository(nil, logger, PlaceholderIndexed), DefaultStatements{}),
		newTestMigrations(t),
	).WithClock(testClock)
...
func TestConfig_String(t *testing.T) {
	assert.Equal(
		t,
		"Config: clock=Fixed clock at 2023-03-10 22:04:27 +0000 UTC, db=Database: clock=Fixed clock at 2023-03-10 22:04:27 +0000 UTC, logger=test logger, repo=SQL db with Indexed args, stmts=Default SQL statements, migs=&{testdata db}, tick=5s, timeout=5m0s", //nolint:lll
		newTestConfig(t, nil, PlaceholderIndexed).String(),
	)
}
